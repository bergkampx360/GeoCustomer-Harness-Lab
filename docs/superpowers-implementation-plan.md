...
6. **Shared vs. local MCP config**: the committed, project-scoped `.mcp.json` defines the `postgres` MCP server with `command: "./scripts/mcp-postgres.sh"` — not a literal connection string, and not `${DATABASE_URL}` env-var expansion inside `.mcp.json` itself (Claude Code does not auto-load a project's `.env` for that expansion). The committed wrapper script (`scripts/mcp-postgres.sh`, contains no secrets) loads `DATABASE_URL` from the local, git-ignored `.env` at invocation time, rewrites `localhost` to `host.docker.internal` where needed for the Docker-based MCP server container to reach the host-published Postgres port, and starts `crystaldba/postgres-mcp` in `--access-mode=restricted` (read-only) mode. No credentials are ever committed; `.env.example` documents the expected shape.
